...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON winner_id = team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT name FROM games INNER JOIN teams ON opponent_id = team_id WHERE year = 2014 AND round = 'Eighth-Final';")"
